...
**중요**: 강사가 입력한 원본 내용은 반드시 그대로 포함하고, 그 내용을 중심으로 확장해야 합니다.
원본 텍스트를 절대 삭제하거나 변경하지 마세요."""
...
✅ 필수 요구사항:
1. 강사가 입력한 원본 내용("{user_input}")을 반드시 포함하세요
2. 원본 내용을 중심으로 학습 목표, 진행 내용, 실습 활동을 추가하세요
3. 원본 키워드나 문장을 삭제하거나 변경하지 마세요
4. 자연스럽게 원본 내용이 녹아들도록 작성하세요

📝 작성 형식:
- 원본 내용이 포함된 도입부 작성
- 학습 목표 추가
- 주요 학습 내용 상세화 (원본 키워드 활용)
- 실습/프로젝트 내용 추가
- 학습 성과 정리

{detail_instructions.get(detail_level, detail_instructions['normal'])}

**다시 한번 강조**: "{user_input}" 이 내용은 반드시 결과물에 포함되어야 합니다!
"""
    ...
    try:
        if groq_api_key:
            ...
            # API 키가 없으면 템플릿 기반 생성 (원본 내용 포함)
            detail_templates = {
                'summary': f"""오늘은 {user_input}에 대한 수업을 진행했습니다. 
학생들은 핵심 개념을 이해하고 기본적인 실습을 완료했습니다.""",
                
                'normal': f"""오늘 수업에서는 {user_input}에 대해 학습했습니다.
...
주요 내용:
오늘은 {user_input}를 중심으로 이론과 실습을 병행했습니다. 학생들은 관련 기술의 기본 원리를 이해하고, 실제 프로젝트에 적용하는 방법을 학습했습니다.

학습 성과:
학생들은 {user_input}에 대한 이해도를 높이고 실무 능력을 향상시켰습니다.""",
                
                'detailed': f"""【수업 개요】
오늘 수업에서는 {user_input}에 대해 심도 있게 학습했습니다.

【학습 목표】
1. {user_input}의 기본 개념과 원리 이해
2. 실무 환경에서의 활용 방법 습득
3. 관련 도구 및 기술 활용 능력 향상

【주요 학습 내용】
오늘은 {user_input}를 중심으로 이론 강의와 실습을 진행했습니다. 
- 기본 개념 설명 및 이론 학습
- {user_input}의 실제 활용 사례 소개
- 단계별 실습 프로젝트 진행
- 학생들의 질의응답 및 토론

【실습 활동】
학생들은 {user_input}를 활용한 실습 프로젝트를 진행했습니다. 각자의 아이디어를 바탕으로 작업을 수행했으며, 과정에서 발생한 문제들을 해결하면서 실무 능력을 향상시켰습니다.

【학습 성과 및 피드백】
학생들은 {user_input}에 대한 전반적인 이해도를 높였으며, 실제 프로젝트에 적용할 수 있는 역량을 키웠습니다. 대부분의 학생들이 적극적으로 참여했고, 과제 수행률도 높았습니다.

【향후 계획】
다음 시간에는 오늘 학습한 {user_input}를 심화하여 더 복잡한 프로젝트에 적용하는 방법을 학습할 예정입니다."""
            }
