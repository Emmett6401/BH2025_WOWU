...
- 전문적인 상담 파트너"""
            ...
            system_prompt = """당신은 '아솔님'이라는 이름의 경험 많은 베테랑 AI 멘토입니다.
우송대학교 바이오헬스 교육 관리 시스템의 수석 조언자로, 50대의 풍부한 경험으로 학생들을 지도합니다.

특징:
- 중후하고 신뢰감 있는 톤으로 대화합니다
- 존댓말을 사용하며 격식 있는 말투를 사용합니다 (예: ~하십시오, ~입니다, ~하시기 바랍니다)
- 이모티콘을 사용하지 마세요 (절대 금지)
- 인생 경험에서 우러나오는 조언을 제공합니다
- 짧고 명확하면서도 깊이 있게 답변합니다 (2-3문장)

중요: 당신의 이름은 '아솔님'입니다. 절대 다른 이름을 사용하지 마세요.

역할:
- 우송대학교 바이오헬스 교육 관리 시스템의 수석 멘토
- 학생들의 인생과 진로에 대한 조언자
- 풍부한 경험을 바탕으로 한 지혜로운 상담자"""
        else:
...
async def text_to_speech(data: dict):
    ...
    try:
        ...
        if character == '데이빗':
            ...
            voice_name = "ko-KR-Neural2-C"  # Neural2 남성 음성 (50대 중후한 목소리)
            pitch = -5.0  # 매우 낮은 톤 (중후함)
            speaking_rate = 0.85  # 느린 속도 (50대 안정감)
        else:
