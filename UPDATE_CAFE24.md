# Cafe24 서버 업데이트 가이드 v3.2

## 🚀 간단한 업데이트 방법 (권장)

### 방법 1: 자동 업데이트 스크립트 사용 ⭐

```bash
# 1. Cafe24 서버 SSH 접속
ssh root@114.202.247.97

# 2. 프로젝트 디렉토리로 이동
cd /root/BH2025_WOWU
# 또는
cd /var/www/webapp

# 3. 자동 업데이트 스크립트 실행 (원클릭!)
bash update_server.sh
```

**스크립트가 자동으로 수행하는 작업:**
- ✅ GitHub에서 최신 코드 다운로드 (`git pull origin main`)
- ✅ 기존 Frontend 프로세스 종료
- ✅ Frontend 재시작 (포트 3000)
- ✅ 서비스 정상 작동 확인
- ✅ PWA 파일 확인 (아이콘 8개, manifest.json 등)
- ✅ Nginx 리로드 (필요시)

---

## 🔧 방법 2: 수동 업데이트 (문제 발생 시)

```bash
# 1. 서버 접속
ssh root@114.202.247.97

# 2. 프로젝트 디렉토리로 이동
cd /root/BH2025_WOWU

# 3. GitHub에서 최신 코드 가져오기
git pull origin main

# 4. 현재 실행 중인 Frontend 프로세스 확인
ps aux | grep python | grep 3000

# 5. Frontend 프로세스 종료
pkill -f "python.*3000"
# 또는 PM2 사용 시
pm2 delete frontend-server

# 6. Frontend 재시작
cd /root/BH2025_WOWU/frontend
nohup python3 -m http.server 3000 --bind 0.0.0.0 > /tmp/frontend.log 2>&1 &

# 7. 프로세스 확인
ps aux | grep python | grep 3000

# 8. 서비스 테스트
curl http://localhost:3000/
curl http://localhost:3000/manifest.json
curl -I http://localhost:3000/woosong-logo.png

# 9. Nginx 리로드 (필요시)
nginx -t
systemctl reload nginx
```

---

## 🔍 업데이트 후 확인 사항

### 1. 웹사이트 접속 테스트
```bash
# 로컬 접속
curl http://localhost:3000/

# 외부 접속 (IP)
curl http://114.202.247.97/

# 도메인 접속 (설정된 경우)
curl http://kdt2025.com/
```

### 2. v3.2 신규 파일 확인
```bash
cd /root/BH2025_WOWU/frontend

# 우송대학교 로고 확인 (v3.2 신규 추가)
ls -lh woosong-logo.png

# README.md 버전 확인
grep "v3.2" ../README.md

# index.html 버전 확인
grep "v3.2" index.html
```

### 3. 브라우저에서 확인
1. **버전 확인**: http://114.202.247.97/ 접속 → 헤더에 "KDT교육관리시스템 v3.2" 표시
2. **로고 확인**: 헤더 좌측 상단에 우송대학교 공식 로고 이미지 표시
3. **기능 테스트**:
   - 강사관리 → 이름순 정렬 확인
   - 학생관리 → 5가지 정렬 옵션 확인
   - 상담관리 → 학생 선택 드롭다운 상세 정보 확인
   - 알림 메시지 → 3초 후 자동 숨김 확인

---

## 📱 v3.2 주요 변경사항

### ✨ 새로운 기능
1. **우송대학교 공식 로고**
   - 파일: `woosong-logo.png` (616x295, 15KB)
   - 위치: 헤더 좌측 상단

2. **강사관리 필터 개선**
   - 정렬: 이름순 (기본) / 강사코드순
   - 검색 필드 자동완성 차단
   - 초기 로딩 시 이름순 정렬

3. **학생관리 필터 강화**
   - 5가지 정렬: 이름순/과정순/캠퍼스순/최종학교순/생년월일순
   - 과정 필터 드롭다운

4. **학생 선택 드롭다운 정보 강화**
   - 표시 형식: `이름(코드) - 과정명 - 생년월일 - 최종학교`
   - 적용: 팀관리, 상담관리

5. **사진 아이콘 위치 통일**
   - 모든 테이블 첫 번째 컬럼에 배치

6. **알림 메시지 자동 숨김**
   - 모든 Alert 3초 후 자동 닫힘

---

## 🛠️ 문제 해결

### 문제 1: Git pull 실패
```bash
# 로컬 변경사항 확인
git status

# 로컬 변경사항 버리기 (주의!)
git reset --hard HEAD
git pull origin main

# 또는 로컬 변경사항 백업 후 pull
git stash
git pull origin main
git stash pop
```

### 문제 2: Frontend 시작 실패
```bash
# 로그 확인
tail -f /tmp/frontend.log

# 포트 3000 사용 중인 프로세스 확인
lsof -i :3000
netstat -tulpn | grep 3000

# 강제 종료 후 재시작
fuser -k 3000/tcp
cd /root/BH2025_WOWU/frontend
nohup python3 -m http.server 3000 --bind 0.0.0.0 > /tmp/frontend.log 2>&1 &
```

### 문제 3: 브라우저에서 변경사항이 안 보임
```bash
# 브라우저 강제 새로고침
# Chrome: Ctrl + Shift + R (Windows/Linux)
# Chrome: Cmd + Shift + R (Mac)
# Safari: Cmd + Option + R

# 또는 브라우저 캐시 완전 삭제
# Chrome: 설정 → 개인정보 및 보안 → 인터넷 사용 기록 삭제
```

### 문제 4: Nginx 설정 오류
```bash
# Nginx 설정 테스트
nginx -t

# Nginx 재시작
systemctl restart nginx

# Nginx 로그 확인
tail -f /var/log/nginx/error.log
tail -f /var/log/nginx/access.log
```

### 문제 5: 로고 이미지가 안 보임
```bash
# 파일 존재 확인
ls -lh /root/BH2025_WOWU/frontend/woosong-logo.png

# 파일 권한 확인
chmod 644 /root/BH2025_WOWU/frontend/woosong-logo.png

# 직접 접속 테스트
curl -I http://localhost:3000/woosong-logo.png
```

---

## 📊 서비스 상태 확인

### 프로세스 상태
```bash
# Frontend 프로세스 확인
ps aux | grep python | grep 3000

# Backend 프로세스 확인 (PM2 사용 시)
pm2 list

# Nginx 상태 확인
systemctl status nginx
```

### 네트워크 상태
```bash
# 열려있는 포트 확인
netstat -tulpn | grep -E '(80|3000|8000)'

# 방화벽 설정 확인
iptables -L -n | grep -E '(80|3000|8000)'
```

### 디스크 용량 확인
```bash
# 디스크 사용량
df -h

# 프로젝트 디렉토리 크기
du -sh /root/BH2025_WOWU
```

---

## 🎯 현재 서버 구성

### 서버 정보
- **IP**: 114.202.247.97
- **도메인**: kdt2025.com (설정된 경우)
- **OS**: Linux (Cafe24 서버)
- **웹서버**: Nginx (포트 80)

### 디렉토리 구조
```
/root/BH2025_WOWU/
├── backend/              # FastAPI 백엔드 (포트 8000)
│   ├── main.py
│   └── .env
├── frontend/             # 프론트엔드 (포트 3000)
│   ├── index.html
│   ├── app.js
│   ├── woosong-logo.png  # v3.2 신규 추가
│   └── proxy-server.cjs
├── update_server.sh      # 자동 업데이트 스크립트
└── README.md             # v3.2 업데이트됨
```

### 포트 설정
- **80**: Nginx (외부 접속)
- **3000**: Frontend (Python HTTP Server)
- **8000**: Backend API (FastAPI/Uvicorn)

### 프록시 설정 (Nginx)
```nginx
# Frontend
location / {
    proxy_pass http://localhost:3000;
}

# Backend API
location /api/ {
    proxy_pass http://localhost:8000;
}
```

---

## 📞 추가 지원

### GitHub 저장소
- **URL**: https://github.com/Emmett6401/BH2025_WOWU
- **Branch**: main (프로덕션)
- **Branch**: temp (개발)

### 업데이트 체크리스트
- [ ] SSH 접속 확인
- [ ] Git pull 성공
- [ ] Frontend 재시작
- [ ] 브라우저에서 v3.2 표시 확인
- [ ] 우송대학교 로고 표시 확인
- [ ] 강사관리 정렬 기능 확인
- [ ] 학생관리 필터 기능 확인
- [ ] 알림 메시지 자동 숨김 확인

### 긴급 롤백 (문제 발생 시)
```bash
# 이전 커밋으로 되돌리기
cd /root/BH2025_WOWU
git log --oneline -5
git reset --hard <이전_커밋_해시>

# Frontend 재시작
pkill -f "python.*3000"
cd frontend
nohup python3 -m http.server 3000 --bind 0.0.0.0 > /tmp/frontend.log 2>&1 &
```

---

**최종 업데이트**: 2025-11-17  
**버전**: 3.2  
**문서 작성자**: GenSpark AI Assistant
